...
module "gitops-cp-db2wh-service" {
  source = "github.com/cloud-native-toolkit/terraform-gitops-cp-db2wh-service?ref=v1.2.1"

  channel = var.gitops-cp-db2wh-service_channel
  cluster_ingress_hostname = var.gitops-cp-db2wh-service_cluster_ingress_hostname
  cluster_type = var.gitops-cp-db2wh-service_cluster_type
  common_services_namespace = var.ibm_common_services_namespace
  cpd_namespace = var.cp4d_namespace
  db2_warehouse_channel = var.gitops-cp-db2wh-service_db2_warehouse_channel
  db2_warehouse_version = var.gitops-cp-db2wh-service_db2_warehouse_version
  git_credentials = module.gitops_repo.git_credentials
  gitops_config = module.gitops_repo.gitops_config
  kubeseal_cert = module.gitops_repo.sealed_secrets_cert
  license = var.gitops-cp-db2wh-service_license
